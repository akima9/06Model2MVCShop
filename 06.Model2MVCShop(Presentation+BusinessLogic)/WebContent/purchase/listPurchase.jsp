...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<c:forEach var="purchase" items="${list}">
		...
		<tr class="ct_list_pop">
			...
			<c:set var="trimText" value="${fn:trim(purchase.tranCode)}"/>
				<c:if test="${trimText=='1'}">
					<td align="left">현재 구매완료 상태 입니다.</td>
				</c:if>
				<c:if test="${trimText=='2'}">
					<td align="left">현재 배송중 상태 입니다.</td>
					<td></td>
					<td align="left"><a href="/updateTranCode.do?tranNo=${purchase.tranNo}&tranCode=3">물건도착</a></td>
				</c:if>
				<c:if test="${trimText=='3'}">
					<td align="left">현재 배송완료 상태 입니다.</td>
				</c:if>
			</c:if>
			<c:if test="${purchase.tranCode==null}">
				<td align="left">현재 tranCode가 null인 상태 입니다.</td>
				<td></td>
			</c:if>		
		</tr>
		<tr>
